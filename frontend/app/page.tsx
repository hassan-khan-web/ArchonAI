...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setSubmitStatus("Repository submitted successfully!");
        setRepoUrl("");
      } else {
        setSubmitStatus("Failed to submit repository.");
      }
    } catch (error) {
      setSubmitStatus("Error submitting repository.");
    }
  };
      ...
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm lg:flex">
        ...
        <div className="fixed bottom-0 left-0 flex h-48 w-full items-end justify-center bg-gradient-to-t from-white via-white dark:from-black dark:via-black lg:static lg:h-auto lg:w-auto lg:bg-none">
          <div className="flex place-items-center gap-2 p-8 lg:p-0">
            ...
            <span className={`font-bold ${backendStatus.includes("Disconnected") ? "text-red-500" : "text-green-500"}`}>
              {backendStatus}
            </span>
          </div>
        </div>
